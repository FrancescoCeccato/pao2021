...
std::pair<double, std::string> polar_chart::max() const {
...
std::vector<std::string> polar_chart::chart_info()const{
    ...
    info.push_back("La media delle entrate è " + std::to_string(mean()) + "/" + std::to_string(max().first));
